...
Everything above describes what agentctl already does, except the following,
which this document proposes and a spec will define:

1. **Detached launch as the default**, the `--tmux` opt-in, the explicit
   `--detached` synonym, and preflight requiring tmux only on the tmux path.
   Today `launch` always creates a tmux presentation and preflight requires
   tmux unconditionally (`internal/preflight/preflight.go`).
2. **`agentctl attach ROLE`** — per-role attach, its stream artifact, the
   repaint on attach, and single-viewer refusal. Today `attach` is fleet-level
   and tmux-only.
3. **The `presentation` template field**, including its absence meaning
   detached. The embedded template schema is strict, so this field and its
   schema entry must ship together.
4. **`run --from-template`.**
5. **The post-launch attach hints** shown in Quickstart. Implementing them
   closes [#196](https://github.com/mnbf9rca/agentctl/issues/196), which asks
   for exactly this and is folded into this design's scope.
