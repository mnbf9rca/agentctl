...
- **Arbitrary keystroke injection.** A client sends a protocol version, a validated session and role, and one name from a closed, argument-free operation registry. The request has no field that can hold text, so no option, environment variable, or stdin path lets caller-supplied text reach an agent's terminal (spec §15.5). Structural tests pin that shape.
