...
| 5 | `no-baseline` | The stored process baseline is empty, so agentctl never proved the pane's launch identity. No current-process probe is issued. | Not a claim that the process changed. |
| 6 | `unexpected-process` | The pane's observed root executable differs from the launch baseline, or identity could not be verified for an alive pane. Identity unverifiable is not identity verified. The rendered process is the one **observed now**, not the expected one. | Not proof of compromise; often a wrapper or restart. |
| 7 | `running` | Window, pane, and process identity all match the launch baseline. | Not that the agent is responsive or idle. |
